...
<body>
	...
	<div class="user-info">
		<form id="moveForm" action="/orderHis" method="post" class="compOrder_form">
		<table>
			<tr>
				<th>사용자 아이디</th>
			...
			<tr>
				<th>배송지입력</th>
				<td><input name="userName" type="hidden" value="${SPRING_SECURITY_CONTEXT.authentication.principal.user.username }">
			<!-- 
			<input type="hidden" name="orPayDate" value="${orPayDate}">
			 -->
					<p>
						<input type="text" id="deleveryAddress1" placeholder="우편번호" value="${BMaddress.address1 }" name="orPostNum" readonly> 
					</p>
					<p>
						<input type="text" value="${BMaddress.address2 }" 
							id="deleveryAddress2" readonly placeholder="배달 받으실 주소를 입력해 주세요" name="orRoadAddress" onclick="modifyAddress()">     
					</p>
					<p>
	...
	<div class="detailright">
		...
		<!-- 가격 종합 정보 -->
		<h2>결제정보</h2>
			<input type="hidden" class="individual_orTotalPrice_input" value="${orderList.orTotalPrice}">
			<input type="hidden" name="alNum" class="individual_alNum_input" value="${orderList.alNum}">
		    <input type="hidden" name="caCount" class="individual_caCount_input" value="${orderList.orTotalAmount}">
			...
			<div>
				<p><span class="delivery_price_span">0</span>원</p>
			</div>	
		...
		<!-- 	사용할 회사명: kcp 버튼 -->
				<table>
					<tr>
						<td>
							<button class="order_btn" type="button" onclick="payment('kcp', 'test', 'card')">카드 결제</button>
						</td>
				<!-- 		카드 결제 테스트 -->
				<!-- 		<button type="button" onClick="payment('kcp', 'real', 'card')"> kcp real payment</button> -->
				<!-- 		카드 결제 -->
				 		<td>
				 			<button class="order_btn" type="button" onclick="payment('kcp', 'test', 'vbank')">무통장 입금</button>
						</td>
					</tr>
		 		</table>
