    ...
    align-items: center;">
					...
					<div style="display: inline-block; margin-left: 15%;">
						...
						<div class="iteminfo" style="font-size: 20px;">
							주종 : <span> ${alDTO.kiName }</span><br>
							도수 :<span> ${alDTO.alAbv } %</span><br> 
							용량 :<span>${alDTO.alVolume } ml</span><br>
							구매제한 :<span>최소 1개</span><br>
							상품코드 :<span>${alDTO.alNum }</span><br>
							제조사 :<span>${alDTO.maName }</span><br>
							재료 :<span>${alDTO.alJaeryo}</span><br>
			...
			<div class="detailright">
				<form  action="/orderForm" name="form" method="post">
					...
					<input type="hidden" name="alNum" value="${alDTO.alNum}"/>
 					<div>
						<b>상품명 :</b><br> <input class="alname input-group input-group-sm mb-3" name="alName" type=text value="${alDTO.alName}" readonly></input>
						<input type=hidden ondragstart='return false'  name="sell_price" value="${alDTO.alPrice}">
					</div>
					<div>
						<b>수량 :</b> 
							<input class="alname input-group input-group-sm mb-3" style="width: 120px;" type="text" name="orTotalAmount" id="amount" value="0" size="3" onkeydown="change(this);" onkeyup="change(this);"> 
							<input class="volumebtn btn btn-light" type="button" value=" + " onclick="add();"> 
							<input class="volumebtn btn btn-light" type="button" value=" - " onclick="del();">
					</div>
					<input type="hidden" name="sum" size="11" readonly>
						<br/>
					<div>
						<b>총 금액 : </b><input type="number" id="my_sum" name="orTotalPrice" value="0" readonly/>원
					</div>
					<br />
					<div style="border-radius: 10px; background-color: white; color: #827f7d;">
					...
					<div id="btn_cart1">
						<c:choose>
					      <c:when test="${not empty SPRING_SECURITY_CONTEXT.authentication.principal.user.username }">
							 <button type="button" class="btn_cart btn btn-warning">장바구니 추가</button> 
					         <input class="btn btn-warning" type="submit" value="결제하기" /> 
					      </c:when>
					      <c:otherwise>
					         <button class="btn btn-warning btnlogin" type="button" onclick="location.href='/login';" >로그인하기</button>
					      </c:otherwise>
					   </c:choose>
					</div>
				</div>
...
var radarChart = new Chart(marksCanvas, {
  ...
  options: chartOptions
			...
			<div id="snack">
			<div>
				<img class=imgOne id="imgSw1" src="" /> 
				<img class=imgOne id="imgSw2" src="" /> 
				<img class=imgOne id="imgSw3" src="" /> 
			</div>
			<div>
				<img class=imgTwo id="imgSi1" src="" /> 
				<img class=imgTwo id="imgSi2" src="" /> 
				<img class=imgTwo id="imgSi3" src="" /> 
			</div>
			<div>
				<img class=imgThree id="imgSs1" src="" /> 
				<img class=imgThree id="imgSs2" src="" /> 
				<img class=imgThree id="imgSs3" src="" /> 
			</div>
			<div>
				<img class=imgFour id="imgBo1" src="" /> 
				<img class=imgFour id="imgBo2" src="" /> 
				<img class=imgFour id="imgBo3" src="" />
			</div>
			</div>
		<hr>	
		</div>
 ...
 <div id="map" style="width:600px;height:350px; visibility: hidden;"></div>
	</div>
		...
		<hr>
			<div id="detail">
				<div class="postbox">
					...
					<ul>
						<li>술은 주종 또는 용기의 크기와 소재에 따라 제품을 보호할 수 있는 두께의 종이 박스나 보냉 박스를
							사용하며 냉장 보관이 필요한 술의 경우 아이스팩을 동봉하여 보내드립니다.</li>
						<li>계절이나 상황에 따라 포장의 형태가 변경 될 수 있습니다.</li>
					</ul>
				</div>
				<hr>
				<div id="delivery">
...
<hr>
 ...
 function toggleBtn1() {
	...
	var infowindow = new kakao.maps.InfoWindow({
	    content : '<div style="padding:2px;"><div>${alDTO.maName}</div><div style="font-size: 12px">${alDTO.maAddr}</div></div>' // 인포윈도우에 표시할 내용
	});
	// 인포윈도우를 지도에 표시한다
