	...
	<section>
		<div class="form-group">
			<div class="row" style="text-align: center;">
				<h1 class="page-header" style="margin-bottom: 50px;">《주문 내역
					페이지》</h1>
							...
							<c:otherwise>
								<c:forEach items="${orderInfo}" var="order">
									<tr>
										...
										<td id="image"><img src="${order.alOpic}"
											referrerpolicy="no-referrer" class="img" /></td>
										<td class="aln">${order.alName}</td>
										<td>${order.caCount}병</td>
										<td>${order.orPayStatus}</td>
										<td><fmt:formatNumber value="${order.orTotalPrice}"
												pattern="#,### 원" /></td>
										<td>${order.orRoadAddress}, ${order.orDetailAddress}</td>
									...
									<form class="reviewDataForm" action="/reviewData" method="post">
										...
										<input type="hidden" name="orPayDate"
											value="${order.orPayDate}"> <input type="hidden"
