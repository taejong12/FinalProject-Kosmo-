...
// 인증번호 보낸 뒤 함수
function sendAuthNumFnc1(inputBox){
	inputBox.prop("readonly", false);
	inputBox.focus();
	timer1.start();
	authNum.isSend(true);
...
$(".auth_num_send_phone").click(function(){
	...
	const inputBox = $(this).siblings(".auth_num1");
...
$(".move_modify").click(function(){
	if(!authNum.isSend()) {
		swal("인증처리가 완료되지 않았습니다.");
		return;
	}
	let authNumber = "";
	
	$("input[type=radio]:checked").each(function(){
		authNumber = $(this).siblings(".auth").find(".authn").val(); 
	})
...
$('.findbtn').on('click', function() {
    var valueCheck = $('.findbtn:checked').val(); // 체크된 Radio 버튼의 값을 가져옵니다.
