				...
				<div>
					<div
						...
						<div style="display: inline-block; margin-left: 15%;">
							...
							<hr>
							<b style="font-size: 40px">판매가격 :<span>
									${alDTO.alPrice }원</span></b>
						</div>
					</div>
					<c:choose>
			...
			<div>
				<div class="detailright">
					<form action="/orderForm" name="form" method="post">
						...
						<div>
							<b>총 금액 : </b><input type="number" id="my_sum"
								name="orTotalPrice" value="0" readonly />원
						</div>
						<br />
						<div
							...
							<div id="btn_cart1">
								<c:choose>
									<c:when
										...
										<button type="button" class="btn_cart btn btn-class">장바구니
											추가</button>
										<input class="btn btn-class" id="order-btn" type="submit" value="결제하기" />
									</c:when>
									<c:otherwise>
										<button class="btn btn-class btnlogin" type="button"
...
$("#order-btn").on("click", function(e){
   ...
   if(form.caCount == null || form.caCount == "" || form.caCount <=0){
       ...
       return false;
    	...
    	swal("금액이 0원입니다.", "You clicked the button!", "warning");
       return false;
    } else{
			...
			<c:if test="${!empty dto}">
				...
				<table id="reviewtable">
					...
					<c:forEach items="${dto}" var="dto">
						<tr>
							<td><a href="/reviewDetail?reNum=${dto.reNum}">${dto.alName}</a></td>
